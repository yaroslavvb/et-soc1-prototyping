...
const M=[
 ['Board power, now','whole card · new value per 133 ms · 10 mW','DM_CMD_GET_MODULE_POWER (ettelem: 45 queries/s; the SP refreshes once per pass)','works_now','works now'],
 ['Board power, PMIC average / min / max','whole card · ~2 s moving average; min and max since the last stats reset','DM_CMD_GET_SP_STATS via ettelem (the stock CLI calls it unsupported), or the SPST trace','works_now','works now'],
 ['Rail power: minion cores, SRAM, mesh','3 rails · 1 mW · ~2 s moving average, one record per 133 ms, plus min/max','same snapshot; the SPST trace keeps ~15 min of records with µs stamps','works_now','works now'],
 ...
 ['Temperature','PMIC, IO shire, mean of 34 minion shires · 1 °C · per 133 ms; "low/high" are extremes since reset, not the current spread','DM_CMD_GET_MODULE_CURRENT_TEMPERATURE','works_now','works now'],
 ...
 ['Energy per event','pJ per load, per multiply-add, per mesh hop · needs ≥10⁹ identical events/s for seconds','rail power above a same-temperature baseline ÷ event rate (memory-anatomy and Horace reports)','works_now','works now'],
...
const U=[
 ['Per-shire voltage map under load','Which shires sag under a full-chip load, and by how much (the low/high capture sees droops the polling misses).','Run the map during a load; the 4 KB log buffer wraps each pass, so poll it quickly.','Nothing: tools/ettelem does it.'],
 ['Temperature-controlled baselines','Energy per event to a few percent instead of ±0.8 W per °C of drift.','Log temperature with every sample (ettelem does) and fit it, or interleave runs as the Horace experiment did.','Nothing.'],
 ...
 ['Chosen counter events','Event counts to regress rail power against: energy per L1 miss, per tensor op, per DRAM activate.','patches/0003 (verified in the simulator), or the debug interface\'s CSR write.','A signed firmware image, or an agreed debug-interface session.'],
...
(function(){const MESH={0:[0,0],24:[1,0],9:[2,0],25:[3,0],2:[4,0],11:[5,0],8:[0,1],16:[1,1],1:[2,1],17:[3,1],10:[4,1],19:[5,1],3:[0,2],4:[1,2],13:[2,2],14:[3,2],18:[4,2],27:[5,2],12:[1,3],21:[2,3],22:[3,3],26:[4,3],20:[1,4],29:[2,4],30:[3,4],15:[4,4],23:[5,4],28:[1,5],5:[2,5],6:[3,5],7:[4,5],31:[5,5]};
 const W=520,cw=80,{svg,tip,h}=host('vmap',W,500);const ramp=['--c1'];
 for(const [s,[cx,cy]] of Object.entries(MESH)){const v=D.shires[s];if(!v)continue;const g=el('g',{},svg);const o=0.25+0.75*(v.mnn[0]-516)/5;
  el('rect',{x:10+cx*cw+2,y:10+cy*cw+2,width:cw-4,height:cw-4,rx:8,fill:'var(--c1)',opacity:o.toFixed(2)},g);
  txt(g,10+cx*cw+cw/2,10+cy*cw+30,'shire '+s,'lab','middle');txt(g,10+cx*cw+cw/2,10+cy*cw+48,v.mnn[0]+' mV','lab-strong','middle');txt(g,10+cx*cw+cw/2,10+cy*cw+64,'low '+v.mnn[1],'lab','middle');
  g.addEventListener('mousemove',ev=>{tip.innerHTML=`<b>Shire ${s}</b><br>minion ${v.mnn[0]} mV [${v.mnn[1]}–${v.mnn[2]}]<br>SRAM ${v.sram[0]} [${v.sram[1]}–${v.sram[2]}]<br>mesh ${v.noc[0]} [${v.noc[1]}–${v.noc[2]}]`;tip.style.display='block';const b=h.getBoundingClientRect();tip.style.left=Math.min(ev.clientX-b.left+12,b.width-200)+'px';tip.style.top=(ev.clientY-b.top+12)+'px';});g.addEventListener('mouseleave',()=>tip.style.display='none');}
})();
