...
(function(){
 ...
 const Wf=C.cards[cards[0]].wire; if(Wf&&Wf.random&&Wf.zeros){
  ...
  document.getElementById('wiretext').innerHTML=`<b>One hop of mesh costs ${dr.toFixed(2)} pJ per byte on random data and ${dz.toFixed(2)} on zeros</b> on ${cards[0]}${dr2!=null?' ('+dr2.toFixed(2)+' and '+dz2.toFixed(2)+' on '+cards[1]+')':''}. The difference — <b>${tog.toFixed(0)}${tog2!=null?' and '+tog2.toFixed(0):''} fJ per bit per hop</b> on the two cards — is the switching energy of the wires and router flops themselves, and it is the one number here the cards agree on to a few percent, because the clocking and buffering that the zeros fit carries cancel out of it; the ${dz.toFixed(2)} pJ/B that a hop costs even when no bit changes is that clocking, arbitration and buffering. The intercept, ${Wf.random.intercept_pj_per_byte.toFixed(2)} pJ/B on random data against ${(Wf.random.local_pj_per_byte||0).toFixed(2)} for the shire's own scratchpad, is the array access plus the step of leaving the shire at all.`;
 }
 /* ---- lines and rows ---- */
