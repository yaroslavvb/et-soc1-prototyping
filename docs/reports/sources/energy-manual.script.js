...
/* ---------- KPIs ---------- */
(function(){
 $('k1').textContent=f1(P80)+' W, '+Math.round(100*R.A_leak_80_w/P80)+'% leakage';
 ...
 const cc=D.catalogue&&D.catalogue.cross_card; if(cc){$('k4').textContent=f3(cc.median); $('k4sub').textContent=`median ratio aifoundry3 / aifoundry2 (10–90%: ${f3(cc.p10)}–${f3(cc.p90)})`;}
...
(function(){
 ...
 $('resttext').innerHTML=`Everything below is <em>above</em> this. The fixed part is the law's constant, ${f1(R.P_fix_w)} W. The blocks with no rail sensor (PCIe, the DDR PHY, the IO shire, the regulators) draw about ${f0(rl.unsensed)} W at idle and barely move with temperature: 0.03 W per °C over 71–83 °C in the catalogue's idle gaps. The three metered rails carry the leakage, 0.55 W per °C between them at about 75 °C. The split into ${f1(R.P_fix_w)} W and leakage is a fit with the ${f0(R.T_L_c)} °C shape imposed, not a block-by-block account. What the unsensed blocks spend when a kernel uses them (DRAM traffic through the DDR PHY, the regulators' delivery loss) is counted in the per-event costs below, and <a href="${HUB}#the-unmetered-remainder-attributed">Limits of observability, §4.2</a> attributes it. The leakage answers to temperature, which the kernel sets: ${f2(R.lambda_80_w_per_c)} W per °C at 80 °C.`;
 const LK=D.cards&&D.cards.leakage, ic3=(LK&&LK.idle_curve)||[];
 CK.legend('idle-legend',[{key:'law',label:'the law, fitted on aifoundry2',mark:'line',color:'var(--c1)'},{key:'fix',label:'its constant',mark:'dash',color:'var(--ref)'},
 ...
 CK.frame('idle',{height:W=>W<600?260:320,label:'Idle board power against die temperature: the law and the measured idle bins',draw:f=>{
  ...
  CK.txt(f.svg,f.W-Rr-4,y(R.P_fix_w)-6,'fixed '+f1(R.P_fix_w)+' W','lab','end');
 ...
 const r3=ic3.length?` Rings: the other card's idle at ${Math.min(...ic3.map(p=>p.T))}–${Math.max(...ic3.map(p=>p.T))} °C (22 September), ${tmin-Math.max(...ic3.map(p=>p.T))}–${tmin-Math.min(...ic3.map(p=>p.T))} °C below the coolest fitted bin and ${LK.mean_offset_W>=0?'+':''}${f1(LK.mean_offset_W)} W from the law (the mean of its ${WORD[ic3.length]} temperature bins).`:'';
 $('idlecap').textContent=`Line: the law fitted on 21 September, P = ${f1(R.P_fix_w)} W + ${f1(R.A_leak_80_w)} W·e^((T−80)/${f0(R.T_L_c)}), whose slope at 80 °C is ${f2(R.lambda_80_w_per_c)} W per °C. Dots: every whole-degree idle bin of that session on aifoundry2, ${tmin}–${tmax} °C.${r3} Leakage share: ${Math.round(100*R.curve.find(c=>c.T===60).leak_frac)}% at 60 °C, ${Math.round(100*R.curve.find(c=>c.T===80).leak_frac)}% at 80 °C, ${Math.round(100*R.curve.find(c=>c.T===90).leak_frac)}% at 90 °C.`;
...
(function(){
 ...
 let sl2note='';
 if(sl2&&sl2.curve&&sl2.curve.length){const lo2=sl2.curve[0]; sl2note=` ${CARDS[1]} idles cooler: its SRAM rail reads ${f2(lo2.sram_w)} W at ${lo2.T} °C, where the ${CARDS[0]} fit, extrapolated, says ${f2(lawS(lo2.T))} W${inr.length?'; its points inside the fitted range are the hollow ones':''}.`;}
 ...
 $('sramcap').textContent=`The rail feeding 128 MB of on-chip SRAM, at idle, on ${CARDS[0]}. Fitted with the idle law's 36 °C e-folding imposed: ${f2(fit.P_fix_w).replace('-','−')} W + ${f2(fit.A_leak_80_w)} W·e^((T−80)/36); ${fit.P_fix_w<0?'the negative constant says the rail rises faster than that shape. ':''}`+
  ...
  `At about ${f0(mb)} mW per MB, a byte held in scratchpad for one second leaks at most about ${f0(nj)} nJ — as much as reading it ${rd?nf(Math.round(nj*1000/rd.mean/100)*100):'—'} times (${rd?f1(rd.mean):'—'} pJ per read).`+sl2note;
})();
...
(function(){
 ...
 $('tensor').innerHTML='<thead><tr><th>Tensor unit, 1,024 minions, 80 °C</th><th class="num">pJ per MAC, marginal</th><th class="num">per card</th><th class="num">pJ per MAC, loaded (a2)</th><th class="num">W over idle (a2)</th><th class="num">MACs per second</th></tr></thead><tbody>'+
...
(function(){
 ...
 fr=CK.frame('allinstr',{height:W=>layout(W).H,label:'Energy per instruction, every instruction, by class',draw:f=>{
  ...
  CK.txt(layer,f.W-Ly.Rr,mrow.y+13,`dearest: ${mx.a.n}, ${nf(val(mx.a,'random'))} pJ`,'lab','end');
 ...
 const rel=all.map(a=>SA[`${a.n}/random/h2`].pj_per_op.se/SA[`${a.n}/random/h2`].pj_per_op.mean).sort((a,b)=>a-b);
 ...
 function cap(){$('allcap').textContent=`${all.length} instructions, one dot each at its random-data energy on ${cardLabel()}${st.unit==='lane'?', per lane for the eight-lane units':''}, one row per class. The hollow ring and the line to it are the same instruction on zeros: for the dot in focus, or for all of them with the toggle. `+
  ...
  `Pass-to-pass standard error on one card is ${(100*rel[rel.length>>1]).toFixed(1)}% in the median and ${(100*rel[Math.floor(rel.length*0.9)]).toFixed(1)}% at the 90th percentile; half the range over both cards' passes is ±${(100*half[half.length>>1]).toFixed(1)}% in the median and ±${(100*half[Math.floor(half.length*0.9)]).toFixed(1)}% at the 90th, most of it the difference between the cards.`;}
 cap();
 ...
 $('alltext').innerHTML=`The cheapest instruction is <code>${cheapest.n}</code> at ${f1(cheapest.m)} pJ and the dearest <code>${dearest.n}</code> at ${nf(dearest.m)} pJ, a span of ${f0(dearest.m/cheapest.m)}× (pooled over both cards). `+
...
(function(){
 ...
 $('memtext').innerHTML=
  ...
  `<b>the same bytes written through the L1 cost ${f1(ss.mean/ds.mean)}× more</b> and arrive at a third of the bandwidth: each store allocates its line, so the line is read from DRAM before it is written back and the byte pays for a read and a write. `+
...
(function(){
 ...
 if(rz2('seq')&&rz2('rowmiss')){
  ...
  $('rowtext').innerHTML=`<b>The row pattern does not change the energy per byte</b>: row hits, row misses and the streaming case agree within their pass-to-pass error on both operand sets. `+
   `The controller runs an open-page policy: a row stays open until a refresh (every 3.87 µs) or an access to another row of its bank closes it (<a href="https://spacesheep.dev/@yaroslavvb/et-soc1-memory-anatomy#how-long-a-row-stays-open">Anatomy of a memory access</a>). `+
   ...
   `So either every pattern paid an activation, or an activation is small next to the transfer (one of 20 pJ/B would have shown); for a programmer it makes no difference. `+
...
(function(){
 ...
 $('railscap').textContent=`Random data, aifoundry2, mean of three passes. The split of each burst's power over idle across the three rails the PMIC meters, read from the end of the burst and corrected for the lag of the PMIC's own running average (a time constant of ${taus.length?f1(Math.min(...taus))+'–'+f1(Math.max(...taus))+' s on the two cards':'about a second'}); the remainder has no sensor.`;
 ...
 if(wire6&&si) $('railstext').innerHTML=`<p><b>An instruction’s energy is the core’s</b>: ${Math.round(100*si.parts[0]/si.o)}% of a scalar integer burst is on the minion rail, almost nothing on the SRAM or the mesh, and the rest is what the regulators lose delivering it. <b>A byte fetched across the mesh is mostly wire</b>: six hops away, ${Math.round(100*wire6.parts[2]/wire6.o)}% of the energy is on the mesh rail and ${Math.round(100*wire6.parts[1]/wire6.o)}% on the SRAM that holds it, with the minions that asked for it at ${Math.round(100*wire6.parts[0]/wire6.o)}%.`+
  (dr?` <b>A DRAM byte is mostly off-chip</b>: ${Math.round(100*dr.parts[3]/dr.o)}% of its energy is on no metered rail.`+(U&&U.aifoundry2&&dl?` Fitted over this whole catalogue in <a href="${HUB}#the-unmetered-remainder-attributed">Limits of observability, §4.2–4.3</a>, ${f0(U.aifoundry2.coef.dram_pj_per_byte)} of its ${f0(dl.mean)} pJ/B sit in the DDR PHY, the I/O rail and the DRAM chips, and the rest of the unmetered share is the regulators' delivery loss (${f0(Math.min(...mn))}–${f0(Math.max(...mn))}% of the minion rail's watts, as far as the rails' meters can be trusted) and the fit's residual${dd?`; the same page reads DRAM power from the DDR rail's voltage droop, 1 mV for about ${f1(1/dd.mv_per_dram_offrail_w)} W`:''}.`:''):'')+'</p>';
})();
...
(function(){
 ...
 $('commnote').innerHTML=(rg.shire?(()=>{
    ...
    return `<p class="small">Rings: re-measured on 23 September with the manual's own sampler, ${WORD[RR.passes.rings/2]||RR.passes.rings/2} passes on each card (n = ${rg.shire.n}). The 18 September pair of runs, sampled without the die temperature and so without a leakage correction, is not pooled; the values <a href="https://spacesheep.dev/@yaroslavvb/et-soc1-on-chip-communication">On-chip communication</a> publishes from it read ${f0(d[0])}–${f0(d[d.length-1])}% higher in every configuration (median ${f0(d[d.length>>1])}%). `+
      `The s ↔ s+16 ring starves the service processor's own management path — the sampler's latency rises from 22 ms to ${dr.length?f0(Math.min(...dr))+'–'+f0(Math.max(...dr)):'over 60'} ms and the board reading takes a new value about twice a second instead of six times — so its aifoundry2 passes were dropped and that row is aifoundry3 only. `+
 ...
 $('commtext').innerHTML=`Between the two minions of a pair a byte costs under a picojoule (${f2(pr)} pJ); around a neighbourhood or a shire about ${f1((nb+sh)/2)} pJ; across the mesh ${f0(Math.min(...xs))}–${f0(Math.max(...xs))} pJ`+
  (fit?`: about ${f0(fit.a)} pJ to leave the shire plus ${f1(fit.b)} pJ per mesh hop (a straight line through the six 1 KB rings between shires against their mean distances of ${f1(Math.min(...hx))}–${f1(Math.max(...hx))} hops, r² ${f2(fit.r2)}). <b>Leaving the shire is the biggest single step, but the hops after it are not free.</b> `:'. ')+
...
(function(){
 ...
 $('sync').innerHTML='<thead><tr><th>Event</th><th class="num">Energy</th><th class="num">per card</th><th class="num">Time</th><th>Note</th></tr></thead><tbody>'+
  ...
  `<tr><td>Chip-wide barrier, ${nf(D.sync.barrier_participants||1024)} minions</td><td class="num">≈ ${f0(bar)} µJ of waiting</td><td></td><td class="num">${nf(D.sync.barrier_cycles_chip)} cycles</td><td class="small">derived: 1,024 minions stalled at ${f1(stallW/1024*1e3)} mW (section 2) for its length</td></tr>`+
...
(function(){
 ...
 function measured(){const p=st.preset&&PRE.find(q=>q[0]===st.preset); if(!p)return null; const q=p[2];
  if(q.tensor){const t=tRow(q.tensor,st.o); if(!t||t.idle_w==null)return null; return {tot:t.idle_w+t.over_idle_w,T:80,label:`measured ${f1(t.idle_w+t.over_idle_w)} W`,src:'the 21 September ablation on aifoundry2, launched at 80 °C'};}
 ...
 const fc=CK.frame('calc',{height:W=>W<600?128:118,label:"Board power priced from the tables",draw:f=>{
  ...
  const segs=[['fixed',pz.fix,'var(--ref)',`<b>fixed</b>: ${f1(pz.fix)} W, the idle law's constant (section 1)`],['leakage',pz.leak,'var(--c4)',`<b>leakage at ${st.T} °C</b>: ${f1(pz.leak)} W (section 1)`]]
 ...
 function upd(){fc.redraw(); const pz=price(st.o,st.T), M=measured(), J=pz.tot*st.dur, first=pz.rows[0];
  ...
  let s=`<b>${f1(pz.tot)} W</b>${pz.rows.length?` [${f1(pz.fix+pz.leak+pz.lo)}–${f1(pz.fix+pz.leak+pz.hi)}]`:''} at ${st.T} °C: idle ${f1(pz.fix+pz.leak)} W (${f1(pz.fix)} fixed + ${f1(pz.leak)} leakage)`+pz.rows.map(r=>` + ${f1(r.w)} W ${r.e.short}`).join('')+
 ...
 $('compcap').innerHTML=`How it is priced: the idle law of section 1 at the chosen die temperature, plus, for each row, its energy per event from sections 3–5 (the mean over both cards; the whisker spans the rows' ranges) times its rate. A rate is a fraction of what the card reached running that row alone. Instruction rows share the harts' issue slots, so together they stop at 100%; byte streams may add. Per-instruction costs include the awake core (section 2), so two instruction rows count it twice. The static part carries the idle law's own bar: ±0.2 W on aifoundry2, ${D.cards.leakage?(D.cards.leakage.mean_offset_W>=0?'+':'')+f1(D.cards.leakage.mean_offset_W):'+0.7'} W on aifoundry3 (the mean of its four temperature bins). `+
  `The default is the dense fp32 matmul on random data: ${f1(R.P_fix_w)} W fixed, ${f1(R.A_leak_80_w)} W leakage at 80 °C and ${f1(sw)} W of multiply-adds (section 3.2's ${f2(tb.mean)} pJ per MAC at ${sci(t.per_s)} MAC/s), ${f1(P80+sw)} W, against ${f1(t.idle_w+t.over_idle_w)} W measured; the flip model of section 3.2 prices the same tile at ${f1((D.cards.patterns||[]).find(p=>p.values==='randn').model)} W instead of ${f1(sw)}.`;
 $('comptext').innerHTML=`<b>At 80 °C the static ${f0(P80)} W exceeds the dynamic power of every kernel measured here</b>, ${mxd===t.over_idle_w?"the dense random matmul's":'the largest,'} ${f1(mxd)} W included. The same matmul on zeros draws ${f1(tz.over_idle_w)} W over idle instead of ${f1(t.over_idle_w)}, and its measured loaded cost per flop falls from ${f1(1e12*(t.idle_w+t.over_idle_w)/(2*t.per_s))} to ${f1(1e12*(tz.idle_w+tz.over_idle_w)/(2*tz.per_s))} pJ, almost all of it static. Cooling the die from 80 to 60 °C saves ${f0(P80-lawAt(60))} W whatever runs. <b>On this card the data decides the dynamic energy and the temperature decides the rest.</b>`;
})();
...
(function(){
 ...
 $('relaytext').innerHTML=`No row of section 4 was derived from the relay, so this is an out-of-sample check. Each byte is read once and written once, so each bracket is the mean of a read row and a write row of section 4, from zeros to random data (the relay's data is one constant per slab). The relay reads with 32 B loads through the L1 and writes with tensor stores. It is priced with the 32 B L1 row of section 4.3 for its own scratchpad, the wire read of section 4.3 at ${f1(HH)} hops for the next shire, and, for DRAM, section 4.1's tensor-load row, the nearest the catalogue has. `+
  ...
  out.map(x=>`${inside.length?' and ':''}${x[0]} reads ${f0(100*Math.abs(1-x[1]/(x[1]<x[2][0]?x[2][0]:x[2][1])))}% ${x[1]<x[2][0]?'below':'above'}`).join('')+
  `; the adds and barriers the relay also runs are in no table. Section 7.1 prices the three relays with the same rows.`;
})();
...
(function(){
 ...
 $('cardscap').textContent=`${pts.length} entries, each the mean of three passes on each card: ratio ${f3(rat[0])} to ${f3(rat[rat.length-1])}, 10th–90th percentile ${f3(rat[Math.floor(rat.length/10)])}–${f3(rat[Math.floor(rat.length*0.9)])}, median ${f3(rat[rat.length>>1])}; the shaded band and the solid line are the catalogue's own 10–90% and median over its ${cc.n} configurations. The line at 1 is equality.`+(rr.length?` The ${rr.length} rerun entries of sections 4.2, 5 and 6 (right, their energies in their own units) give ${f3(rr[rr.length>>1])} in the median (${f2(rr[0])}–${f2(rr[rr.length-1])}).`:'')+` The tensor-unit transfer of 22 September found ${f3(scale||0.924)} for the same pair of cards, with aifoundry3 running 5 mV higher — the scale is the card, not the operating point.`;
 /* the five lowest and five highest ratios, as buttons that move the focus to their point */
 const ex=pts.slice().sort((a,b)=>a.r-b.r), lo=ex.slice(0,5), hi=ex.slice(-5).reverse(), box=$('cards-extremes');
 const bl=(lab,arr)=>{const p=document.createElement('p'); p.className='controls'; p.style.alignItems='center'; const s=document.createElement('span'); s.textContent=lab; p.appendChild(s);
  arr.forEach(q=>{const b=document.createElement('button'); b.type='button'; b.textContent=`${q.k} ${f2(q.r)}`; b.addEventListener('click',()=>{const n=byKey[q.k]; if(n)n.focus();}); p.appendChild(b);}); box.appendChild(p);};
 bl('Lowest ratios:',lo); bl('Highest:',hi);
})();
