...
const bt=(c,fn)=>c?`<b>${fn(c.mean)}</b> <span class="small">[${fn(c.lo)}–${fn(c.hi)}]</span>`:'—';
const pcs=(c,fn)=>c?Object.keys(c.per_card).sort().map(h=>`a${h.slice(-1)} ${fn(c.per_card[h].mean)} ± ${fn(c.per_card[h].se||0)}`).join('<br>'):'—';
...
(function(){
 ...
 const cc=D.catalogue&&D.catalogue.cross_card; if(cc){document.getElementById('k4').textContent=`${f3(cc.median)} [${f3(cc.p10)}–${f3(cc.p90)}]`;}
...
(function(){
 ...
 document.getElementById('rails').innerHTML='<thead><tr><th>Idle at 73 °C, by rail</th><th class="num">W</th><th class="num">Share</th></tr></thead><tbody>'+
  ...
  `<tr><td><b>Board</b> <span class="small">(300 samples over 20 hours, sd 0.04 W; rails sd below 0.01 W)</span></td><td class="num"><b>${f2(rl.board)}</b></td><td></td></tr></tbody>`;
})();
...
(function(){
 ...
 document.getElementById('awake').innerHTML='<thead><tr><th>Minions awake, doing the least they can</th><th class="num">pJ per instruction</th><th class="num">per card</th><th class="num">W over idle, 1,024 minions</th><th class="num">per minion</th></tr></thead><tbody>'+
  ...
  `<tr><td>1,024 minions stalled in a load that never returns (the hot line)</td><td class="num">—</td><td class="num small">${hot?pcs(hot,f2):''}</td><td class="num">${hot?bt(hot,f2):'1.41'}</td><td class="num">${hot?f1(hot.mean/1024*1e3):'1.4'} mW</td></tr>`+
  `<tr><td>Activity term under a dense matmul</td><td class="num">—</td><td></td><td class="num">26.2</td><td class="num">25.6 mW</td></tr></tbody>`;
})();
...
(function(){
 ...
 document.getElementById('instrcap').textContent='Both harts of every minion issuing the instruction back to back; the multiply and the transcendentals are multi-cycle and issue at a quarter to an eighth of the rate. Each bar is the mean over three shuffled passes on each of two cards; the whisker is the range those six passes spanned.';
 ...
 const ia=q('add','zeros'),fa=q('fadd.s','zeros'),vz=q('fadd.ps','zeros'),vr=q('fadd.ps','random'),fm=q('fmadd.ps','random'),ex=q('fexp.ps','random');
 ...
 document.getElementById('instrtext').innerHTML=
  `<b>An integer add is the cheapest thing a core does, ${f1(ia.mean)} pJ on zeros</b> [${f1(ia.lo)}–${f1(ia.hi)}], almost all of it the awake core. `+
  ...
  `The datapath energy is the same in both to within the bars; the tensor unit saves instruction issue. Transcendentals are the dearest instructions on the chip: <code>fexp.ps</code> is ${f0(ex.mean)} pJ for eight lanes at a quarter of the rate.`;
 const TB=D.tensor.bars||{};
 document.getElementById('tensor').innerHTML='<thead><tr><th>TensorFMA, 16×16×16, 1,024 minions, 80 °C</th><th class="num">pJ per MAC, marginal</th><th class="num">per card</th><th class="num">pJ per MAC, loaded (a2)</th><th class="num">W over idle (a2)</th><th class="num">MACs per second</th></tr></thead><tbody>'+
  D.tensor.rows.map(t=>{const b=TB[t.config]; return `<tr><td>${t.label}</td><td class="num">${b?bt(b,f3):'<b>'+f3(t.pj_marginal)+'</b>'}</td><td class="num small">${b?Object.keys(b.per_card).sort().map(h=>`a${h.slice(-1)} ${f3(b.per_card[h].mean)}`).join('<br>')+(b.cards>1?'':'<br>(a2 only)'):''}</td><td class="num">${f2(t.pj_loaded)}</td><td class="num">${f2(t.over_idle_w)}</td><td class="num">${t.per_s.toExponential(2)}</td></tr>`;}).join('')+
  '<tr><td colspan="6" class="small">Bars on the fp32 rows span the ablation’s two runs on aifoundry2 and the 22 September transfer’s runs on both cards (n = 4); fp16 and int8 were run twice on aifoundry2, and their bar is that run-to-run spread, under 1%.</td></tr></tbody>';
...
(function(){
 ...
 document.getElementById('memcap').textContent='Log scale. Tensor loads and stores bypass the caches; the L1 rows are hits in a 256 B buffer; the last bar is a plain vector store streaming to DRAM through the L1. Whiskers: the range over three shuffled passes on each of two cards.';
 document.getElementById('memtab').innerHTML='<thead><tr><th>Path</th><th class="num">zeros pJ/B</th><th class="num">random pJ/B</th><th class="num">random / zeros</th><th class="num">GB/s</th><th class="num">per card, random</th></tr></thead><tbody>'+
  paths.map(p=>{const z=cb(p[0],p[2]),r=cb(p[1],p[2]); if(!(z&&r))return ''; const bps=S2[p[1]]?S2[p[1]].bytes_per_s.mean:0;
   return `<tr><td>${p[3]} <span class="small">(${p[4]})</span></td><td class="num">${bt(z,f2)}</td><td class="num">${bt(r,f2)}</td><td class="num">${f2(r.mean/z.mean)}×</td><td class="num">${bps?(bps/1e9).toFixed(0):'—'}</td><td class="num small">${pcs(r,f2)}</td></tr>`;}).join('')+'</tbody>';
 ...
 if(lv&&lv.dram){
  const LV=[['l1','L1 hits','256 B per hart, 2,048 harts'],['l2','L2','256 KB per shire (L2 is 512 KB)'],['l3','L3','768 KB per shire, 24 MB in all (L3 is 32 MB)'],['dram','DRAM','256 MB in all'],['scp-local','own scratchpad','2 MB of the shire’s own L2 scratchpad'],['scp-remote','remote scratchpad','2 MB of the scratchpad 16 shires away']];
  document.getElementById('memold').innerHTML='<thead><tr><th>Level</th><th>Working set</th><th class="num">pJ/B</th><th class="num">per card</th></tr></thead><tbody>'+
   LV.map(r=>lv[r[0]]?`<tr><td>${r[1]}</td><td class="small">${r[2]}</td><td class="num">${bt(lv[r[0]],f2)}</td><td class="num small">${pcs(lv[r[0]],f2)}</td></tr>`:'').join('')+`<tr><td colspan="4" class="small">Plain vector loads streaming over a working set sized to each level, both harts of every minion, ${RR.passes.levels} passes at a pinned 600 MHz. The 18 September run of the same experiment had the governor free and its clock at 0.67–0.77 GHz on the larger sets; it is superseded.</td></tr></tbody>`;
  document.getElementById('memoldsum').textContent='Reads by level: L1, L2, L3, DRAM and the scratchpads by plain vector loads (memhier, re-run at 600 MHz on both cards)';
 } else {
...
(function(){
 ...
 document.getElementById('comm').innerHTML='<thead><tr><th>Ring, 1 KB messages unless said</th><th>What moves</th><th class="num">pJ/B</th><th class="num">per card</th><th class="num">GB/s aggregate</th></tr></thead><tbody>'+
  D.comm.rows.map(r=>`<tr><td>${r.ring}</td><td class="small">${r.what}</td><td class="num">${val(r)}</td><td class="num small">${rg[r.ring]?pcs(rg[r.ring],f2):'a2, 2 runs'}</td><td class="num">${r.gb_s.toFixed(0)}</td></tr>`).join('')+
  [['hop','slab handed to the next shire through its scratchpad','write where the next shire reads, read there (E25)'],['dram','the same through DRAM','write to DRAM, read back'],['scp','the same kept in this shire’s scratchpad','write and read in place']].map(m=>
   `<tr><td>${m[1]}</td><td class="small">${m[2]}</td><td class="num">${rr[m[0]]?bt(rr[m[0]],f1):'<b>'+f1(rp[m[0]].pj_per_byte)+'</b>'}</td><td class="num small">${rr[m[0]]?pcs(rr[m[0]],f1):'a2 only'}</td><td class="num">${(rp[m[0]].bytes_per_s/1e9).toFixed(0)}</td></tr>`).join('')+
  (rg.shire?`<tr><td colspan="5" class="small">Rings: re-measured on 23 September with the manual's own sampler, ${RR.passes.rings/2} passes on each card (n = ${rg.shire.n}); the 18 September pair of runs, sampled without the die temperature, is not pooled and agrees to within 10%. The s ↔ s+16 ring starves the service processor's own management path — the sampler's latency triples and the board reading is held for seconds — so its aifoundry2 passes were dropped and that row is aifoundry3 only. Relay: 22 September and three warm passes on each card (n = ${rr.dram?rr.dram.n:'—'}).</td></tr>`:'')+'</tbody>';
 const xs=D.comm.rows.filter(r=>r.ring.startsWith('xshire')&&!r.ring.includes('c4')).map(r=>rg[r.ring]?rg[r.ring].mean:r.pj_per_byte);
 const sh=rg.shire?rg.shire.mean:D.comm.rows.find(r=>r.ring==='shire').pj_per_byte, hop=rr.hop?rr.hop.mean:rp.hop.pj_per_byte, dram=rr.dram?rr.dram.mean:rp.dram.pj_per_byte;
 document.getElementById('commtext').innerHTML=`Inside a neighbourhood a byte costs under a picojoule; inside a shire about ${f1(sh)} pJ; across the mesh ${f0(Math.min(...xs))}–${f0(Math.max(...xs))} pJ, roughly flat in distance. <b>The step is leaving the shire, not the hops after that.</b> Small messages cost more per byte: the per-message overhead is a few hundred cycles of both harts. Handing a slab to the next shire through its scratchpad is ${f0(dram/hop)}× cheaper than the DRAM round trip; the DRAM relay carries the widest bar of the three because its 4–5 W over idle rides on a path with no rail sensor, measured against a board idle that drifts.`;
})();
...
(function(){
 ...
 document.getElementById('sync').innerHTML='<thead><tr><th>Event</th><th class="num">Energy</th><th class="num">per card</th><th class="num">Time</th><th>Note</th></tr></thead><tbody>'+
  `<tr><td>Global atomic, one line, 1,024 requesters</td><td class="num">${hn.contended?bt(hn.contended,f1)+' nJ':'<b>'+f1(at.contended.nj_per_op)+' nJ</b>'}</td><td class="num small">${hn.contended?pcs(hn.contended,f1):'a2 only'}</td><td class="num">${f0(at.contended.cycles_per_op)} cycles each at the bank</td><td class="small">the bank serialises; every requester stalls; the host shire loses its memory path</td></tr>`+
  ...
  `<tr><td>Chip-wide barrier, 1,024 minions</td><td class="num">≈ ${f0(bar)} nJ of waiting</td><td></td><td class="num">${D.sync.barrier_cycles_chip.toLocaleString()} cycles</td><td class="small">derived: 1,024 minions stalled at 1.4 mW for its length</td></tr>`+
  `<tr><td>FLB + credit barrier, one shire</td><td class="num">—</td><td></td><td class="num">237 cycles</td><td class="small">nocbench</td></tr>`+
  `<tr><td>TensorReduce + broadcast, 32 minions</td><td class="num">—</td><td></td><td class="num">432 cycles</td><td class="small">nocbench</td></tr>`+
  (hn.contended?`<tr><td colspan="5" class="small">Hot line: 22 September and three warm passes on each card (n = ${hn.contended.n}). The contended row’s bar is wide because the whole chip stalled draws only about 1.4 W over idle, and the per-operation figure divides that small number by a rate the bank fixes at one per 10 cycles.</td></tr>`:'')+'</tbody>';
})();
...
(function(){
 ...
 document.getElementById('compcap').textContent=`Random data. Per flop that is ${f1(1e12*tot/(2*t.per_s))} pJ loaded and ${f2(t.pj_marginal/2)} pJ marginal${tb?' ['+f2(tb.lo/2)+'–'+f2(tb.hi/2)+' over runs and cards]':''}; ${Math.round(100*P80/tot)}% of the energy of the most arithmetic-dense thing this chip does is static. The static part carries the idle law’s bar: ±0.2 W on this card, +0.7 W on the other.`;
 ...
 const rz=(p,s)=>{const z=cb(`${p}/${s?'scp':'dram'}/zeros`),r=cb(`${p}/${s?'scp':'dram'}/random`); return z&&r?[z.mean,r.mean]:[g(p,'zeros',1,s).pj_per_byte,g(p,'random',1,s).pj_per_byte];};
 const w1=cb('wire/hop1/zeros'),w1r=cb('wire/hop1/random'); const rem=w1&&w1r?[w1.mean,w1r.mean]:[mr['scp-remote'].pj_per_byte,mr['scp-remote'].pj_per_byte];
 const pd=pr(rz('tload',false),rz('tstore',false)), ps=pr(rz('tload',true),rz('tstore',true)), ph=pr(rem,rz('tstore',true));
 document.getElementById('relaycheck').innerHTML='<thead><tr><th>The relay of 22 September, predicted from section 4</th><th class="num">predicted pJ/B, zeros … random</th><th class="num">measured</th></tr></thead><tbody>'+
  `<tr><td>intermediate in DRAM</td><td class="num">${f0(pd[0])} … ${f0(pd[1])}</td><td class="num">${rr2.dram?bt(rr2.dram,f1):'<b>'+f1(rp.dram.pj_per_byte)+'</b>'}</td></tr>`+
  `<tr><td>in the shire's own scratchpad</td><td class="num">${f1(ps[0])} … ${f1(ps[1])}</td><td class="num">${rr2.scp?bt(rr2.scp,f2):'<b>'+f2(rp.scp.pj_per_byte)+'</b>'}</td></tr>`+
  `<tr><td>in the next shire's scratchpad</td><td class="num">${f1(ph[0])} … ${f1(ph[1])}</td><td class="num">${rr2.hop?bt(rr2.hop,f2):'<b>'+f2(rp.hop.pj_per_byte)+'</b>'} <span class="small">(a little above: the add and the barrier)</span></td></tr></tbody>`;
})();
...
(function(){
 ...
 const Wf=C.cards[cards[0]].wire; if(Wf&&Wf.random&&Wf.zeros){
  ...
  document.getElementById('wiretext').innerHTML=`<b>One hop of mesh costs ${dr.toFixed(2)} pJ per byte on random data and ${dz.toFixed(2)} on zeros</b> on ${cards[0]}${dr2!=null?' ('+dr2.toFixed(2)+' and '+dz2.toFixed(2)+' on '+cards[1]+')':''}. The difference — <b>${tog.toFixed(0)}${tog2!=null?' and '+tog2.toFixed(0):''} fJ per random bit per hop</b> on the two cards — is the data-dependent energy of the links and routers: part of it is bits that differ from one flit to the next, and part is the ones carried, which cost even when they do not change (<a href="https://spacesheep.dev/@yaroslavvb/et-soc1-heat-per-mm">Heat per millimetre</a> separates the two with chosen bit patterns and converts them to fJ per bit·mm). It is the one number here the cards agree on to a few percent, because the clocking and buffering that the zeros fit carries cancel out of it; the ${dz.toFixed(2)} pJ/B that a hop costs on all-zero data is that clocking, arbitration and buffering. The intercept, ${Wf.random.intercept_pj_per_byte.toFixed(2)} pJ/B on random data against ${(Wf.random.local_pj_per_byte||0).toFixed(2)} for the shire's own scratchpad, is the array access plus the step of leaving the shire at all.`;
 }
 /* ---- lines and rows ---- */
 ...
 if(fz&&fr&&gz&&gr){
  ...
  const c32z=cb('l1fill/stride32/zeros'),c64z=cb('l1fill/stride64/zeros'),c32r=cb('l1fill/stride32/random'),c64r=cb('l1fill/stride64/random');
 ...
 (function(){
  ...
  const unmetText=uf?` <b>Attributed:</b> over the whole catalogue the unmetered part of a burst fits as ${(100*uf.coef.minion).toFixed(0)}% of the minion rail's watts (a delivery loss), ${(100*uf.coef.sram).toFixed(0)}% of the SRAM rail's, ${(100*uf.coef.noc).toFixed(0)}% of the mesh's (more than a regulator: the memory shires' logic on an unmetered rail works when the mesh moves bytes to them) and ${uf.coef.dram_pj_per_byte.toFixed(0)} pJ per DRAM byte off-rail, to rms ${uf.rms_w.toFixed(2)} W over ${uf.n} bursts${U.aifoundry3?' ('+(100*U.aifoundry3.coef.minion).toFixed(0)+'%, '+(100*U.aifoundry3.coef.sram).toFixed(0)+'%, '+(100*U.aifoundry3.coef.noc).toFixed(0)+'% and '+U.aifoundry3.coef.dram_pj_per_byte.toFixed(0)+' pJ/B on aifoundry3)':''}.`:'';
  const droopText=dd?` <b>A droop meter for DRAM:</b> the memory shires' Moortec voltage monitors report the 0.8 V DDR rail every 133 ms (<code>die_mv.ddr</code>, ${dd.idle_die_mv.ddr.toFixed(0)} mV at idle), and it droops ${dd.mv_per_dram_offrail_w.toFixed(2)} mV per watt of off-rail DRAM power (rms ${dd.rms_mv.toFixed(2)} mV): 1 mV ≈ 1.2 W of DRAM at 10 Hz, from a sensor that was always there. How this and every other instrument could be pushed further is <a href="https://spacesheep.dev/@yaroslavvb/et-soc1-limits-of-observability">the limits of observability</a>.`:'';
  if(wire6&&si) document.getElementById('railstext').innerHTML=`<b>An instruction’s energy is the core’s</b>: ${Math.round(100*si.parts[0][1]/si.o)}% of a scalar integer burst is on the minion rail, almost nothing on the SRAM or the mesh, and the rest is what the regulators lose delivering it. <b>A byte fetched across the mesh is mostly wire</b>: six hops away, ${Math.round(100*wire6.parts[2][1]/wire6.o)}% of the energy is on the mesh rail and ${Math.round(100*wire6.parts[1][1]/wire6.o)}% on the SRAM that holds it, with the minions that asked for it at ${Math.round(100*wire6.parts[0][1]/wire6.o)}%.`+(dr?` <b>A DRAM byte is mostly off-chip</b>: ${Math.round(100*(dr.parts[3][1])/dr.o)}% of its energy is on no metered rail at all — the DDR PHY and the memory itself.`:'')+unmetText+droopText;
 })();
 ...
 const nb=[0,1,2,3].map(k=>S[`neigh/${k}/random`]); if(nb.every(v=>v)){
  document.getElementById('neightab').innerHTML='<thead><tr><th>Which neighbourhood reads the shire’s own scratchpad (random data)</th><th class="num">pJ/B, both cards</th><th class="num">per card</th><th class="num">GB/s</th></tr></thead><tbody>'+
   nb.map((v,k)=>{const c=cb(`neigh/${k}/random`);return `<tr><td>neighbourhood ${k}, minions ${8*k}–${8*k+7}</td><td class="num">${c?bt(c,f2):v.pj_per_byte.mean.toFixed(2)}</td><td class="num small">${c?pcs(c,f2):v.pj_per_byte.se.toFixed(2)}</td><td class="num">${(v.bytes_per_s.mean/1e9).toFixed(0)}</td></tr>`;}).join('')+'</tbody>';
 }
})();
