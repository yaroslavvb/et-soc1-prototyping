...
(function(){
 ...
 t.innerHTML='<thead><tr><th>At 0.485 V, 400 MHz; the model on the loaded mesh</th><th class="num">mesh rail, fJ per hop</th><th class="num">fJ per mm</th><th class="num">board power, fJ per hop</th><th class="num">fJ per mm</th></tr></thead><tbody>'+
  ...
  `<tr><td>random bit, data part</td><td class="num">${bar(UN.per_hop.random_bit_data,f0)}</td><td class="num">${bar(UN.random_bit_data,f1)}</td><td class="num">${bar(UB.per_hop.random_bit_data,f0)}</td><td class="num">${bar(UB.random_bit_data,f1)}</td></tr>`+
  `<tr><td>per bit, data-independent</td><td class="num">${bar(UN.per_hop.fixed_per_bit,f0)}</td><td class="num">${bar(UN.fixed_per_bit,f1)}</td><td class="num">${bar(UB.per_hop.fixed_per_bit,f0)}</td><td class="num">${bar(UB.fixed_per_bit,f1)}</td></tr>`+
...
(function(){
 ...
 function update(){
  ...
  if(m){const d=1-m.mean/e, word=d>0.1?'far below':d>0.005?'slightly below':d<-0.005?'above':'on';
   h+=`<p style="margin:4px 0 0">Measured: <b>${f0(m.mean)}</b> fJ${m.sizes?` (four block sizes, ${f0(m.lo)}–${f0(m.hi)})`:` [${f0(m.lo)}–${f0(m.hi)}], n = ${m.n}`}: ${word==='on'?'on the plane':`${word} the plane (${f0(100*Math.abs(d))}%)`}${m.sizes||preset==='b256'?'; a first-run pattern the model was not fitted to':''}.</p>`;}
...
(function(){
 ...
 const f=CK.frame('cont',{label:'Energy per payload byte against distance, with every flow on its own links and on a loaded mesh',height:W=>W<600?300:330,draw(f){
  ...
  const dj=W_.disjoint_flows[key]; if(dj&&dj.wsep_d1_4&&dj.wu){const u=dj.wsep_d1_4.random_minus_zeros_fj_per_bit_hop, l=dj.wu.random_minus_zeros_fj_per_bit_hop, uz=dj.wsep_d1_4.zeros_fj_per_bit_hop, lz=dj.wu.zeros_fj_per_bit_hop;
   setText('contcap',`Per bit per hop over 1–4 hops, ${meterName(key)}: the data-dependent part costs ${f0(u.mean)} fJ with every flow on its own links against ${f0(l.mean)} on the loaded mesh, the data-independent part ${f0(uz.mean)} against ${f0(lz.mean)}. Solid: random data; dashed: zeros; bars: the range over six passes${sel?`; rings: the ${sel}-hop points, the distance on the map`:''}.`);}
 }});
...
(function(){
 ...
 CK.legend('alt-leg',[{key:'s',label:'blocks of 16–128 B',mark:'box',color:'var(--c1)'},{key:'l',label:'256 B blocks',mark:'box',color:'var(--c2)'},{key:'p',label:'the first run\'s model, with 0 or 1 difference per bit',mark:'dash',color:'var(--ink)'}]);
 const f=CK.frame('alt',{label:'Cost per hop of blocks of N bytes alternately all-zero and all-one',height:W=>W<600?260:280,draw(f){
  ...
  const pred=n=>s0+b*0.5+a*(n===256?1:0);
  ...
  Ns.forEach((n,i)=>{const e=PT[`alt:${n}`]; if(!e)return; const s=e[key].slope, g=CK.el('g',{},f.svg), tl=n===256?1:0;
   ...
   CK.tip(f,g,`<b>blocks of ${n} B</b>, alternately all-0 and all-1, ${meterName(key)}<br>${f3(s.mean)} pJ/B per hop [${f3(s.lo)}–${f3(s.hi)}]<br>on one lane consecutive lines are i and i+4: ${n<=128?'the same block value, no differences':'opposite values, every bit differs'}<br>dashed: the model with ${tl} difference${tl?'':'s'} per bit, ${f3(pred(n))}`);
   nodes.push(g);});
  CK.keynav(f,nodes);
 }});
...
(function(){
 ...
 const sepLin=Math.max(CHK.wsep_max_off_line['wsep/p0/noc_pj_per_byte'],CHK.wsep_max_off_line['wsep/p0.5/noc_pj_per_byte']);
 setText('disttext',
 ...
 const below=k=>{const m=W_.model.v1[srcOf(k)], p=m.s0_pj_per_byte_hop.mean+m.ones_fj_per_one_bit_hop.mean*8/1000*0.5;   // no-transition prediction, s0 + b/2
  return [16,32,64,128].filter(n=>W_.patterns[`alt:${n}`]).map(n=>1-W_.patterns[`alt:${n}`][k].slope.mean/p);};
 const blk=[16,32,64,128].every(n=>W_.patterns[`alt:${n}`])?` The 16–128 B blocks also come in ${pcs(below(NK))} below the model's no-transition prediction on the mesh rail (${pcs(below(BK))} on board power), a first-run pattern the model was not fitted to.`:'';
 if(ab&&an){
 ...
 const xyv=hosts.map(xy12), dir=xyv.every(q=>q>0)?' — y below x on both cards —':xyv.every(q=>q<0)?' — y above x on both cards —':'';
 setText('axistext',`(The x-only and y-only pairs differ by ${pcs(xyv.map(Math.abs))} over one and two hops${dir} but they also differ in link sharing, so they cannot separate the two.)`);
