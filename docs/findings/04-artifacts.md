...
| A19 | `docs/reports/2026-09-18-et-soc1-ridge-points.html` — "Ridge Points": peak compute divided by each level's bandwidth, i.e. the operations per byte a kernel needs before compute rather than memory sets its speed, with what that takes for blocked matmul and for batch sizes, the limits that act like ridge points, energy balance points and an A100 comparison. Derived from the four 2026-09-18 reports with no card time. Unlike the reports above it is not built by `scripts/build-report.py`: the prose lives in the HTML and `scripts/ridge-points.py --embed` rewrites only its embedded JSON | the 2026-09-18 measurements (matmul, memory hierarchy, on-chip communication, sparsity) | [et-soc1-ridge-points](https://spacesheep.dev/@yaroslavvb/et-soc1-ridge-points) `dd341b0b-a52e-4e23-b8b3-101a83119133` | public (asked for on 2026-09-24) |
...
## Cross-links between the reports (2026-09-24)

Every report now links back to the hub (A2) from its byline: "· part of the measurement reports". The six
source-built pages whose committed HTML predates the current template (dvfs-leakage, horace-experiment,
hot-line, on-chip-relay, power-temperature, why-low-power) were patched in both `<name>.body.html` and the
built page, rather than rebuilt: a rebuild would also have pulled in the template's newer section-anchor
script and math CSS, republishing changes that are not this one. Their next real rebuild keeps the link.
The two spaces with no source in this repository — the L2 mainline-starvation brief
(`49ca367f-886f-4c0a-b472-12d8fc449300`) and the spatial-temperature brief
(`bc391cfe-64e2-4f36-884c-9bbfcb267de8`) — were patched on the published page only, so whoever holds their
source should add the same byline clause there.

## A19 — The ridge-point tool (2026-09-18)

- `scripts/ridge-points.py`: reads the raw data of the four 2026-09-18 reports (`docs/reports/data/*/runs.jsonl`,
  `results.json`) and their embedded JSON, and prints every ridge point, the clock elasticity of each level, the
  limits that act like ridge points, the energy balance points and the A100 comparison. `--embed <html>` rewrites
