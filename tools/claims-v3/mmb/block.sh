...
# aifoundry2 heats the die to >= 76 C before each E1 workload, before ridge-X1 and before the load step;
# aifoundry3 sleeps 60 s before the load step. Everything is in tools/claims-v3/mmb/README.md.
...
[ "$CARD" = aifoundry2 ] && { [ -e "$HEATER" ] || missing="$missing $HEATER"; }
if [ -n "$missing" ]; then
...
# V3_DRY: print every device command on the block's own stderr (fd 9), also from calls whose output goes to files.
# A local override of lib.sh's dry hold10 (which prints to the redirected stderr); the real hold10 is untouched.
exec 9>&2
if [ -n "${V3_DRY:-}" ]; then hold10() { echo "DRY hold10: $* (cwd $PWD)" >&9; }; fi
...
printf '{"pass":%s,"smoke":%s,"card":"%s","e1":[%s],"x1":[%s],"mmbench":"%s","memprobe":"%s"}\n' "$PASS" \
  "$SMOKE_JSON" "$CARD" "$(json_list $E1_ORDER)" "$(json_list $X1_ORDER)" "$MMB_LAUNCHER" "$MEMPROBE" \
  > "$OUT/order.json"
...
if [ -n "$SMOKE" ] && [ "$CARD" = aifoundry2 ]; then   # the heater binary itself, one 1 s launch
...
for w in $E1_ORDER; do
  ...
  if [ "$CARD" = aifoundry2 ] && [ -z "$SMOKE" ]; then heat_to 76 "$d/heat.jsonl" || note "heat_to 76 gave up before $w"; fi
  ...
  if [ -n "${V3_DRY:-}" ]; then e1run > "$d/run.log" 2>&9   # dry: the runner's DRY lines on the block's stderr
  else e1run > "$d/run.log" 2>&1; fi || note "e1 $w: runner failed"
  stop_sampler
  python3 "$HERE/mmbench_power_v3.py" finish --out "$d" --telemetry "$d/telemetry.jsonl" --card "$CARD" >> "$d/run.log" 2>&1
...
X=$OUT/x1; mkdir -p "$X"
if [ "$CARD" = aifoundry2 ] && [ -z "$SMOKE" ]; then heat_to 76 "$X/heat.jsonl" || note "heat_to 76 gave up before x1"; fi
...
if [ "$CARD" = aifoundry2 ]; then
  [ -z "$SMOKE" ] && { heat_to 76 "$T/heat.jsonl" || note "heat_to 76 gave up before the load step"; }
else
  [ -z "$SMOKE" ] && nap 60
fi
...
python3 "$HERE/passcheck.py" "$OUT" --card "$CARD" ${SMOKE:+--smoke} > "$OUT/passcheck.json" 2> "$OUT/passcheck.err"
pc=$?
