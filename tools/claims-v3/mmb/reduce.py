...
def load_e1(wd, card):
    ...
    return {"runs": runs, "kept": kept, "dropped": [q["launch"] for q in runs if q["_drop"]],
            "mean_w": x["mean_w"], "idle_before_w": idle_b, "tflops": x["tflops"], "gflops_per_w": x["gflops_per_w"],
...
def load_card(root, card):
    ...
    for k, d in pass_dirs(root, "mmb"):
        ...
        P = {"pass": k, "dir": d, "status": json.load(open(bj)).get("status") if os.path.exists(bj) else None,
             "e1": {}, "x1": {}, "smoke": load_smoke(os.path.join(d, "smoke")), "thermal": None}
        for w in WORKLOADS:
...
def mmb_a(D):
    ...
    for card in CARDS:
        ...
        for w in WORKLOADS:
            ...
            c[w] = {"passes": len(E), "launches": len(ks), "band": [r(b[0], 2), r(b[1], 2)] if b else None,
...
def mmb_b(D):
    ...
    return item("MMB-b", pc, "every kept L2 timed launch's tflops inside 9.501-9.521 / 19.00-19.04 / 71.70-71.86; every kept timed "
                ...
                "it_test_code_loading record rc 0 and exact; >= 3 kept passes per mode", oc, reading)
...
def mmb_d(D):
    pc, cross, ok, enough = {c: {} for c in CARDS}, {}, True, True
    alpha = 0.01 / 3
    for w in L2:
    ...
    return item("MMB-d", pc, "per L2 mode, Welch t on pass values of board GFLOP/s per W (gflops_per_w), two-sided, alpha 0.01/3; "
                "holds if significant with mean(a3)/mean(a2) >= 1.08, in all three modes", oc, reading, {"cross": cross})
...
def mmb_e(D):
    ...
    return item("MMB-e", pc, "per card, one-sided 99% t (t_0.99,n-1) on pass values of int8 board GOP/s per W: the A100 wins if "
                ...
                "a3 1.00-1.25)", oc, reading, {"consequence": cons})


def mmb_f(D):
    pc, holds = {c: {} for c in CARDS}, {}
    for card in CARDS:
        ok, enough = True, True
        for w in WORKLOADS:
            ...
            if w == DRAM:
                ...
                s["claim"] = "+0.5 to +2 W, 99% interval above 0" if card == "aifoundry2" else "rise > 0 (no band registered for a3)"
                if s["n"] >= NEED:
    ...
    return item("MMB-f", pc, "per card and workload, pass values of mean(last 1 s) - mean(first 1 s) of the mean_w window (after the "
                ...
                "interval inside +-0.5 W", oc, reading)
    ...
    for card in CARDS:
        ...
        for P in D[card]["passes"]:
            if not P["thermal"]:
                continue
            try:
                o = x1_reduce.reduce_pass(P["thermal"], SHORT[card])
...
def mmb_t(D):
    per = load_step(D)
    kept = {c: [o for o in per[c] if o.get("valid")] for c in CARDS}
    dropped = {c: [o["pass"] for o in per[c] if not o.get("valid")] for c in CARDS}
    items = []
    ...
    def add(name, pc, oc, test, reading, hint, extra=None):
    ...
    add("P4-remainder", pc, oc, "DRAM-phase remainder rise over the cool1 baseline (W): 99% t excludes 0 and mean in 3-7",
        f"DRAM remainder {oc}", ["pt-spatial-26", "pt-spatial-29", "pt-spatial-31"])
    pc, holds = {}, {}
    for c in CARDS:
    ...
    add("P5-dram", pc, oc, "die_mv.ddr below the idle trend in the DRAM phase (mV): 99% t excludes 0 and mean in 2-5",
        f"DDR droop under DRAM load {oc}", ["pt-spatial-38", "pt-spatial-39"])
    pc, holds = {}, {}
    for c in CARDS:
    ...
    add("P6", pc, oc, "minion on-die droop per W of minion-rail rise (mV/W): 99% t excludes 0 (band only reported)",
        f"minion droop {oc}", ["pt-spatial-V02"])
    pc, holds = {}, {}
    for c in CARDS:
        rows = []
        for o in kept[c]:
    ...
    add("P10", pc, oc10, "aifoundry2 only: pass mean of the die's drop in the 40 s after the matmul in 4-8 C (aifoundry3 reported)",
        "cooling after the matmul (a2)", ["pt-spatial-27"])
    ...
    summary = item("MMB-T", {c: {"passes_reduced": len(per[c]), "kept": len(kept[c]), "dropped": dropped[c],
                                 "errors": [o for o in per[c] if "error" in o]} for c in CARDS},
                   ...
                   {"busy_minus_idle_slope_a2_not_decided": bmi, "per_pass": {c: per[c] for c in CARDS}})
...
def main():
    ...
    items = [mmb_a(D), mmb_b(D), mmb_c(D), mmb_d(D), mmb_e(D), mmb_f(D)] + mmb_x1(D) + mmb_t(D)
    ...
    if not a.quiet:
        for i in items:
            print(f"{i['item']:16s} {i['outcome']:15s} {i['reading']}")
    return 0
