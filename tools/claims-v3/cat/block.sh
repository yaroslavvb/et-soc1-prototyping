...
#   CAT_HOLD_C=<C> ...                                          # aifoundry3 hot passes: override the hold temperature
#
# The pass number fixes the arm and condition (the registered order, arm A and arm B alternating on each card):
...
#   W = aifoundry2 warm (no hold: wait until <= 82 C, then heat_to 76), H = hold hot (aifoundry2 --hold-hot 88,
#   aifoundry3 --hold-hot Tmax-1 from V3-IDLE), C = aifoundry3 cool (no hold, no heater).
# Arm B = dramrow2, tload/dram, tstore/dram, l1fill/stride32, l1fill/stride64, tload/scp, fence, nop (23
#   configurations), run_catalogue's default --burst 3 --gap 5, --seed 30<k>; aifoundry2 preheated to 76 C.
...
A2_HOLD=88; A2_WARM_MAX=82; PREHEAT_MAX_LAUNCHES=40
...
# aifoundry2: 88 C. aifoundry3: Tmax - 1, Tmax = the highest minshire in V3-IDLE's telemetry on this card
...
hold_target() {
  if [ "$CARD" = aifoundry2 ]; then echo "$A2_HOLD"; return 0; fi
...
if [ -n "$need_heater" ]; then
  ...
  else HOLD=$(hold_target) || { echo "aifoundry3 hot pass: no V3-IDLE telemetry under $DATA_ROOT/idle/p*/ and no CAT_HOLD_C" >&2; exit 2; }
  fi
fi

# ---- local helpers (lib.sh's heat_to is aifoundry2-only with a fixed 76 C / 150-launch loop) ------------------------
...
json.dump({"exp": "cat", "card": "$CARD", "pass": $PASS, "smoke": bool("$SMOKE"), "arm": "$ARM", "cond": "$COND", "k": $K,
           ...
           "heater": "$HEATER", "n_cfg": $NCFG, "max_s": $MAXS}, open(sys.argv[1], "w"), indent=1)
EOF
...
if [ -z "$SMOKE" ]; then
  if [ "$CARD" = aifoundry2 ]; then
    if [ "$COND" = W ]; then wait_cool "$A2_WARM_MAX" 600 "$OUT/preheat.jsonl" || true; fi
    heat_to 76 "$OUT/preheat.jsonl" || { finish fail "aifoundry2 did not reach 76 C"; exit 1; }
  fi
...
rargs=("$OUT" --root "$V3_ROOT" --only "$ONLY" --passes 1 $TIMING --seed "$SEED" --host-bin "$HOST_BIN"
       --tel-live "$OUT/telemetry.jsonl.raw")
