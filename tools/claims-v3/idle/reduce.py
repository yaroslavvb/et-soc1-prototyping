...
Layout read: <data>/<card>/idle/p<N>/{telemetry.jsonl.gz, launches.jsonl, marks.jsonl, cycle.json, block.json}.
Passes 1-9 are short cycles (900 s of cooling), 11-19 IDLE-LONG cycles (5400 s aifoundry2, 2700 s aifoundry3).
A pass is kept when block.json says ok, it is not a dry run, and marks.jsonl has cool_start and cool_end.
...
def load_pass(pdir, card=None):
    ...
    samples = []           # (t_ms, T, P, mhz, minion, sram, noc)
...
def rule_a_bins(p, cool_s=CYCLE_COOL_S):
    """Whole-degree idle bins of one cycle (rule A). Returns (rows by T, counters)."""
...
def ftm_idle_samples(p):
    """rule F: flip_thermal_model.py step 2a's own idle rule over the whole pass."""
    import numpy as np
...
def k_idle_samples(p):
    """rule K: >= 20 s after the last burst, inside the first 900 s of cooling, 600 MHz, 3 s smoothing."""
    import numpy as np
    tt, P, T, mhz, act = ftm_grid(p)
    ...
    ok = (tt >= last_end + 20.0) & (tt >= t0 / 1000.0) & (tt <= t1 / 1000.0) & (mhz == 600)
...
def item_0(C):
    ...
    ok = len(tm) >= MIN_REPEATS
    holds = (all(60 <= t <= 66 for t in tm) if ok else None)
    pc = {A3: {"tmax_per_cycle": tm, "n": len(tm), "band": [60, 66], "all_inside": holds,
               "heat_reason_per_cycle": [c["heat_reason"] for c in cyc]}, A2: na(A2)}
    ...
    return {"item": "IDLE-0", "claims": ["energy-manual-22"], "per_card": pc,
...
def item_a(C):
    ...
    pc = {A3: {"n": n, "offset_W": so, "resid_slope_W_per_C": ss, "cycles": info,
               "info_prediction": {"offset_point": 0.61, "every_bin_in_0.3_0.9": bool(allbins) and all(0.3 <= v <= 0.9 for v in allbins),
                                   "slope_point_within_0.03": ss["mean"] is not None and abs(ss["mean"]) <= 0.03}},
          A2: na(A2)}
    out = card_verdicts({A3: holds}, [A3])
    ...
    return {"item": "IDLE-a", "claims": ["energy-manual-145", "energy-manual-01", "dvfs-63", "energy-manual-16", "horace-lowpower-124"],
...
def item_b(C):
    ...
    offs = [mean([r["resid"] for r in c["bins"].values()]) for c in C[A2] if c["bins"]]
    so = stat(offs, band)
    n = len(C[A2])
    ...
    pc = {A2: {"n": n, "bins": pb, "testable_bins": testable, "bins_outside_band": bad,
               "info_cycle_offset_W": so, "cycles_T_range": {c["pass"]: [min(c["bins"]), max(c["bins"])] for c in C[A2] if c["bins"]}},
          A3: na(A3)}
    ...
    return {"item": "IDLE-b", "claims": ["energy-manual-06", "energy-manual-15", "dvfs-61"], "per_card": pc,
...
def item_c(C):
    ...
    pc = {A2: {"n": n, "unsensed_slope_W_per_C_74_88": su, "upper_bound_below_0.15": holds,
               "info_metered_rail_slope_W_per_C_75_80": sr, "info_point_prediction": {"unsensed": 0.055, "rails": "0.51 +- 0.07"},
               "cycles": info}, A3: na(A3)}
    pc[A3]["info_unsensed_slope_per_cycle"] = rnd([slope([r["T"] for r in c["bins"].values() if "unsensed" in r],
                                                         [r["unsensed"] for r in c["bins"].values() if "unsensed" in r]) for c in C[A3]])
    out = card_verdicts({A2: holds}, [A2])
    ...
    return {"item": "IDLE-c", "claims": ["energy-manual-11", "energy-manual-12", "energy-manual-13", "energy-manual-163"], "per_card": pc,
...
def item_d(C):
    ...
    pc = {A2: {"n": n, "passes_with_73C_bin": used, "components_W": st, "info_board_W": stat(boards), "reference_22Sep": SPLIT_22SEP,
               "cycles_without_73C_bin": [c["pass"] for c in C[A2] if c["pass"] not in used]}, A3: na(A3)}
    out = card_verdicts({A2: holds}, [A2])
    ...
    return {"item": "IDLE-d", "claims": ["energy-manual-18", "dvfs-62", "hub-080"], "per_card": pc,
...
def item_e(C):
    ...
    pb = per_bin(C[A3], "sram_excess", lo_T=51, tmax_cap=True)
    for r in pb.values():
    ...
    pc = {A3: {"n": n, "sram_slope_W_per_C": ss, "excess_over_a2_sram_law_by_bin": pb, "testable_bins": testable,
               "bins_below_0.8": bad, "cycles": info}, A2: na(A2)}
    out = card_verdicts({A3: holds}, [A3])
    ...
    return {"item": "IDLE-e", "claims": ["energy-manual-19", "energy-manual-20", "energy-manual-22"], "per_card": pc,
...
def item_f(C):
    a3_70 = [c["bins"][70]["unsensed"] for c in C[A3] if 70 in c["bins"] and "unsensed" in c["bins"][70]]
    a2_70 = [c["bins"][70]["unsensed"] for c in C[A2] if 70 in c["bins"] and "unsensed" in c["bins"][70]]
    ...
    pc = {A3: {"n": len(C[A3]), "n_cycles_with_70C_bin": len(a3_70), "tmax_per_cycle": [c["tmax"] for c in C[A3]], "own_temperature": own(C[A3])},
          A2: {"n": len(C[A2]), "n_cycles_with_70C_bin": len(a2_70), "own_temperature": own(C[A2]), "reference_W_at_70C": 14.7}}
    if met:
    ...
    return {"item": "IDLE-f", "claims": ["energy-manual-163"], "per_card": pc, "condition_met": met,
...
def item_k(C):
    cyc = [c for c in C[A2] if c["tmax"] is not None and c["tmax"] >= 86]
    ...
    pc = {A2: {"n": n, "cycles_from_86C": [c["pass"] for c in cyc], "cycles_below_86C": [c["pass"] for c in C[A2] if not (c["tmax"] is not None and c["tmax"] >= 86)],
    ...
    return {"item": "IDLE-k", "claims": ["dvfs-05", "dvfs-06", "energy-manual-147", "energy-manual-149"], "per_card": pc,
            ...
            "outcome": out, "decision": pc[A2]["decision_busy_leakage_above_Kanter_30pct"], "reading": rd}


def item_L(L):
    pc, holds = {}, {}
    a2 = [x for x in L[A2] if x.get("fit")]
    ...
    pc[A2] = {"n": len(a2), "best_T_L_C": s_tl, "A80_W": s_a, "slope80_W_per_C": s_sl,
    ...
    holds[A2] = (s_tl["inside_band"] and s_a["inside_band"] and s_sl["inside_band"]) if len(a2) >= MIN_REPEATS else None
    a3 = [x for x in L[A3] if x.get("fit")]
    s_56 = stat([x["fit"]["slope56"] for x in a3], (0.22, 0.38))
    s_off = stat([x["offset"] for x in a3 if x.get("offset") is not None], (0.3, 1.1))
    pc[A3] = {"n": len(a3), "slope56_W_per_C": s_56, "offset_from_law_W": s_off,
              "per_pass": [{"pass": x["pass"], **x["fit"], "offset": rnd(x.get("offset")), "profile": x["profile"]} for x in a3]}
    holds[A3] = (s_56["inside_band"] and s_off["inside_band"]) if len(a3) >= MIN_REPEATS and s_off["n"] >= MIN_REPEATS else None
    out = card_verdicts(holds, [A2, A3])
    rd = (f"IDLE-LONG: aifoundry2 T_L {fmt_ci(s_tl, 0)} C, A80 {fmt_ci(s_a, 1)} W, slope80 {fmt_ci(s_sl, 3)} W/C ({len(a2)} passes, split {pc[A2]['leakage_split'].split(':')[0]}); "
          f"aifoundry3 slope56 {fmt_ci(s_56, 3)} W/C, offset {fmt_ci(s_off)} W ({len(a3)} passes)")
    return {"item": "IDLE-L", "claims": ["energy-manual-10", "horace-lowpower-084", "horace-lowpower-087", "horace-lowpower-088", "horace-lowpower-139",
...
def reduce_all(data, out_path):
    ...
    for card, dirs in discover(data).items():
        for d in dirs:
            p = load_pass(d, card)
            why = keep_reason(p)
            ...
            if card == A2 and c["tmax"] is not None and c["tmax"] >= 86:
                Ts, Ps, Traw, nk = k_idle_samples(p)
                if nk >= BIN_MIN_N:
            ...
            if p["variant"] == "long":
                ...
                if nf >= BIN_MIN_N:
                    ...
                    lb, _ = rule_a_bins(p, cool_s=0)
                    x["offset"] = mean([r["resid"] for r in lb.values()]) if lb else None
                L[card].append(x)
    items = [item_0(C), item_a(C), item_b(C), item_c(C), item_d(C), item_e(C), item_f(C), item_k(C), item_L(L)]
    res = {"experiment": "V3-IDLE", "plan": "docs/reports/data/2026-09-25-claims-v3/PLAN3.md section V3-IDLE",
           "data": os.path.abspath(data), "passes": passes,
           "cycles_used": {c: [x["pass"] for x in C[c]] for c in CARDS}, "long_passes_used": {c: [x["pass"] for x in L[c]] for c in CARDS},
           # PLAN3 order rule "cycles on at least two different days": reported, not a reduction rule
           "info_days_of_kept_cycles_utc": {c: sorted({r["date_utc"] for r in passes[c] if r["kept"] and r["date_utc"]}) for c in CARDS},
           "constants": {"law": LAW, "sram_law_a2": SRAM_LAW_A2, "split_22sep": SPLIT_22SEP, "busy_80": BUSY_80, "T_L_grid": TL_GRID,
                         "bin_min_n": BIN_MIN_N, "cycle_cool_s": CYCLE_COOL_S},
           "items": items}
    with open(out_path, "w") as f:
        json.dump(res, f, indent=1, default=float)
    for it in items:
        print(f"{it['item']:7s} {it['outcome']:14s} {it['reading']}")
    return res
...
def check_pass(pdir):
    ...
    note = (f"{cyc.get('variant')}: {res['bursts']} bursts ({res['heat_reason']}), Tmax {res['tmax']} C; "
            ...
            f"T {res.get('cool_T')}, off600 {res.get('off600_cool')}" + (f", bins {res['bins'][0]}-{res['bins'][-1]}" if res.get("bins") else ""))
    if warnings:
