...
#   governor-free cards (aifoundry2, aifoundry1-c0/-c1) hot 83 C launch / 86 C preheat, then cool 76 / 79 (above
#   the governor's window); the pinned aifoundry3 hot 65 / 68, then cool 55 / 60.
#   bash tools/claims-v3/x5/block.sh <pass 1..3> [--smoke]        (aifoundry1: V3_DEVICE=0 or 1 selects the card)
...
case $CARD in
  ...
  *) echo "x5: no leakage slope for card $CARD" >&2; exit 2 ;;
esac
b=$(( pass - 1 ))
...
NOTE=
if [ "$mode" = --smoke ]; then
  # Smallest real check (~12 s of card time, up to ~25 s on a cool governor-free card): both arms' session paths
  # (sampler stop and restart between them),
