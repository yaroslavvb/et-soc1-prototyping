...
# start (aifoundry2: heat to 84 C, launch at 80 C; aifoundry3: 57 / 55 C), 10 Hz sampler for the block.
#   bash tools/claims-v3/ablb/block.sh <pass 1..3> [--smoke]
...
case $CARD in
  aifoundry2) TARGET=80; PREHEAT=84; LEAK=0.81; LAUNCH=80.9; export ABL_CAP_S=2100 ;;
  aifoundry3) TARGET=55; PREHEAT=57; LEAK=0.55; LAUNCH=55.8; export ABL_CAP_S=1500 ;;
esac
...
if [ "$mode" = --smoke ]; then
  # Smallest real check (~30 s of card time, ~45 s on a cool aifoundry2 with heat_to 76): one heater burst; 3 s
