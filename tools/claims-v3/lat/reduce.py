...
"""V3-LAT reducer: the registered prediction items LAT-M1 ... LAT-R of PLAN3.md §2 "V3-LAT", exactly as registered.
    ...
    python3 tools/claims-v3/lat/reduce.py --data <dir> --out <verdicts.json> [--no-search] [--card aifoundry2]

<dir> holds aifoundry2/ and aifoundry3/, each laid out like DATA_ROOT (build/claims-v3/<card>/lat/p<N>/...).
...
class Unit:
    def __init__(self, card, path):
        ...
        self.a2 = card == "aifoundry2"
        self.clock = Clock(path)

    def f(self, name):
        return os.path.join(self.path, name)

    def keep(self, t0, t1=None, ghz=None):
        """The registered drop rule for one launch (aifoundry3: always kept)."""
        if not self.a2:
            return True
...
class NocPass:
    ...
    def kept_file(self, name):
        ...
        if not tops:
            return False
        return all(self.U.keep(r.get("t_start_ms"), r.get("t_end_ms"), rec_ghz(r)) for r in tops)
...
def sp_kept(U, rs):
    return bool(rs) and all(U.keep(r.get("t_start_ms"), r.get("t_end_ms"), rec_ghz(r)) for r in rs)
...
def pass_values(C, unit_names, fn):
    ...
    for K, us in sorted(C.passes.items()):
        ...
        kept = all(r is not None for _, r in used) and all(
            U.keep(r.get("t_start_ms"), r.get("t_end_ms"), rec_ghz(r)) for U, r in used)
...
def lat_s5(C):
    ...
    for K, U in C.units("sp"):
        ...
        kept = all(U.keep(r.get("t_start_ms"), r.get("t_end_ms"), rec_ghz(r)) for r in used)
    ...
    for label, U in C.short:
        ...
        if not st or not rf:
            continue
        k = all(U.keep(r.get("t_start_ms"), r.get("t_end_ms"), rec_ghz(r)) for r in st[:1] + rf[:1])
...
def lat_g(C):
    ...
    for K, U in C.units("sg"):
        ...
        for i in SG_REF:
            ...
            clock_ok = lr is not None and (not U.a2 or U.clock.brackets(lr["t0_ms"], lr["t1_ms"]) is True)
...
class HotPass:
    def __init__(self, K, U):
        ...
        for r in jlines(U.f("sweep.jsonl")):
            ...
            if warm != 5:
                key += f"|w{warm}"
            r["_kept"] = U.keep(r.get("t_start_ms"), r.get("t_end_ms"), rec_ghz(r))
            self.by[key] = r
...
def lat_h(C):
    ...
    hs = [s["holds"] for s in subs.values()]
    ...
    return {"n": min(s["n"] for s in subs.values()) if subs else 0, "holds": holds, "sub": subs,
...
def lat_r(C):
    ...
    for K, U in C.units("rl"):
        ...
        for r in jlines(U.f("sweep.jsonl")):
            if r.get("test") != "relay":
                continue
            recs[rl_key(r)] = (r["gb_s"], U.keep(r.get("t_start_ms"), r.get("t_end_ms"), rec_ghz(r)), r.get("ok"))
    ...
    complete = n_off >= NEED and nv >= NEED and n_rat >= NEED and min(B2[1], B8[1]) >= NEED and len(ch) == 9
...
def main():
    ...
    cards = {c: Card(a.data, c) for c in CARDS}
    items = []
    for name, fn, test in ITEMS:
        ...
        items.append({"item": name, "claims": plan.get(name, {}).get("claims", []),
                      ...
                      "outcome": oc, "reading": reading(name, pc, oc)})
    out = {"experiment": "V3-LAT", "data": os.path.abspath(a.data),
           "cards": {c: {"blocks": cards[c].blocks, "skipped": cards[c].skipped,
                         ...
                         "short_blocks": [s[0] for s in cards[c].short]} for c in CARDS},
           "items": items}
