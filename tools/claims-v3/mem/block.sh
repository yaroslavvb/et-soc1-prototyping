...
#
#   bash tools/claims-v3/mem/block.sh <pass> [--smoke]          # from the tree root (aifoundry2: this repo; aifoundry3: ~/nekko)
#   V3_DRY=1 bash tools/claims-v3/mem/block.sh <pass>           # no device access: prints every device call
#
# A pass: arena base (--info); the op lists generated off-card (seeds 100+k, 200+k, 20+k); [aifoundry2] heat to 76 C;
# the 10 Hz sampler; the 11 programs t_raw t_glitch t_rawodd ladder decomp l3map msmap bits refresh refresh_jit
# pagetimeout in shuf order, each its own process; ladder + decomp from requesters 7, 24, 31 (--hart 64*S); the sampler
# stopped (SIGTERM); then, in passes 1-3 (or a later pass while fewer than 3 wake-up probes are kept on this card):
# [aifoundry2] heat top-up to 76 C, a 1 s sample, the wake-up probe, a 1 s sample. Every device process is under
# timeout 10 (hold10) and memprobe's --budget 8. Passes > 5 are conditional re-runs: they do nothing when the card
# already has 5 kept passes and 3 kept wake-up probes.
...
# The module's use count catches a holder that ps does not name (plan 2.13: 0, or 1 while only our sampler runs).
if [ -z "${V3_DRY:-}" ]; then
  u=$(lsmod 2>/dev/null | awk '$1=="et_soc1"{print $3}')
  if [ -n "$u" ] && [ "$u" != 0 ]; then log "et_soc1 use count $u: the card is held, not starting"; exit 3; fi
fi

# The op lists must be the pre-registered generator's (the reducer's prereg/mp/gen_ops.py): on aifoundry3 the tree is a
# synced copy, and an older gen_ops.py would give other programs under the same names.
...
# take up to 150 launches). Returns 0 at >= 76 C, 1 if it gave up, 3 if another user arrived. aifoundry2 only.
heat76() {
  local out=$1 t i
  [ "$CARD" = aifoundry2 ] || return 0
  if [ -n "${V3_DRY:-}" ]; then heat_to 76 "$out"; return 0; fi
  for i in $(seq 1 150); do
    t=$(die_c); echo "{\"t_ms\":$(now_ms),\"die_c\":${t:-0}}" >> "$out"
    [ -n "$t" ] && [ "$t" -ge 76 ] && return 0
...
if [ "$MODE" = smoke ]; then
  ...
  if [ "$CARD" = aifoundry2 ]; then   # one heater launch (the heat_to command) instead of a full heat
...
if ! start_sampler "$OUT/telemetry.jsonl" 400; then
  # aifoundry2 drops a pass without telemetry, so do not spend the card on it; aifoundry3 does not drop on the clock
  [ "$CARD" = aifoundry2 ] && { block_end fail "sampler did not start"; exit 1; }
  log "sampler did not start; aifoundry3 continues (its clock is recorded, not a drop rule)"
fi
...
if [ "$WAKE" = 1 ]; then
  ...
  hrc=0
  if others_present; then hrc=3; else [ "$CARD" = aifoundry2 ] && { heat76 "$W/heat.jsonl"; hrc=$?; }; fi
  if [ $hrc = 3 ]; then
    ...
    # aifoundry2 drops a probe without a clock reading, so it does not spend the card on one; aifoundry3 runs it anyway
    if sample1s "$W/pre.jsonl" || [ "$CARD" = aifoundry3 ]; then
...
print(("%s; %s; %d program failures" % (x1, wk, len(pf))).replace('"', "").replace("\\", ""))
EOF
)
if [ -n "${V3_DRY:-}" ]; then block_end ok "dry run"; exit 0; fi
