...
- `docs/report/` is the shareable write-up, published as a private space at https://spacesheep.dev/@yaroslavvb/et-soc1-testdrive.
  Edit `index.html`, then run `npx --yes spacesheep deploy docs/report` to update the same space; `.spacesheep.json` pins it.
...
- `patches/` holds local fixes to et-platform: a broken third-party fetch, and gp-sdk launchers that can't boot sysemu.
  See `patches/README.md`. `external/` holds the upstream clones and is gitignored.
